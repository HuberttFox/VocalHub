  ...
  return (
    <article className="catalog-card group">
      <RemoteImage
        ...
        fallbackLabel="封面不可用"
        height={640}
        src={[song.coverUrlThumb, song.coverUrlOriginal]}
        width={640}
      />
      ...
      <h2 className="mt-4 text-2xl font-semibold tracking-[-0.025em] text-[var(--text-primary)]">
        <Link className="card-link" href={`/songs/${song.id}`}>
          {song.title}
        </Link>
      </h2>
      {alternate && (
      ...
      <p className="mt-4 line-clamp-2 text-[var(--text-secondary)]">
        {song.artistString}
      </p>
      {context && (
        <p className="mt-3 text-sm text-[var(--accent-soft)]">{context}</p>
      )}
      <SongMeta
