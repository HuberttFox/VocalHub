...
- 显式 ID、全量 seed、activity 增量和删除 reconciliation worker；durable manifest、checkpoint 与 resume 支持。
...
所有 mode 都必须显式指定。`incremental` 要求至少一次成功 seed；`resume` 只处理 durable manifest 中未完成的 item。命令返回非零表示 run 未完全成功。同步完成后启动：
...
- manifest 创建后可使用 `resume`；discovery、item processing 或 finalization 失败时 run 保持 RUNNING 并记录错误，供 operator 修复后继续。checkpoint 只在 discovery 完整且没有 FAILED item 时原子推进。
...
1. 为 seed、incremental 与 reconciliation 增加外部 scheduler 和部署级 worker service。
